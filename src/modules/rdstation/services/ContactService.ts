...
class ContactService extends ApiResource {
   ...
   * status | String | granted or declined (only when category is communications)
   */
  upsert(identifier: string, value: string, payload: object) {
    return this.request(
      'PATCH',
  ...
  segmentations(identifier: string) {
    return this.request(
      'GET',
      `/platform/segmentations/${identifier}/contacts`,
    );
  }

  async countAllLead() {
    const allSegmentations = await this.request(
      'GET',
      `/platform/segmentations`,
    );

    const [{ id }] = allSegmentations.data.segmentations;

    return this.getAllLeadsBySegmentationIdPaginated(id);
  }

  async countAllLeadByFunil(): Promise<Array<{ stage: string, quantity: number | undefined, percentil: number | string }>> {
    const allSegmentations = await this.request(
      'GET',
      `/platform/segmentations`,
    );

    const funil = allSegmentations.data.segmentations;
    funil.shift();
    const dataReturn: Array<{ stage: string, quantity: number | undefined, percentil: number | string}> = [];

    await Promise.all(
      funil.map(async (stage: any) => {
        const { id, name } = stage;
        dataReturn.push({
          stage: name,
          quantity: await this.getAllLeadsBySegmentationIdPaginated(id),
          percentil: 0,
        });
      }),
    );
  ...
  private async getAllLeadsBySegmentationIdPaginated(
    id: string,
    page = 1,
    totalQuantity = 0,
    ...
    const leads = await this.request(
      'GET',
      `/platform/segmentations/${id}/contacts?page=${page}&page_size=125`,
    );

    const quantityLeads = leads.data.contacts.length;

    if (quantityLeads === 0) {
      return totalQuantity;
    }

    totalQuantity += quantityLeads;

    return this.getAllLeadsBySegmentationIdPaginated(
      id,
      page + 1,
      totalQuantity,
    );
