...
class ContactService extends _ApiResourceService.default {
  /**
   * Get contact by UUID
   *
  ...
  segmentations(identifier) {
    return this.request('GET', `/platform/segmentations/${identifier}/contacts`);
  }

  async countAllLead() {
    const allSegmentations = await this.request('GET', `/platform/segmentations`);
    const [{
      id
    }] = allSegmentations.data.segmentations;
    return this.getAllLeadsBySegmentationIdPaginated(id);
  }

  async countAllLeadByFunil() {
    const allSegmentations = await this.request('GET', `/platform/segmentations`);
    const funil = allSegmentations.data.segmentations;
    funil.shift();
    const dataReturn = [];
    await Promise.all(funil.map(async stage => {
      const {
        id,
        name
      } = stage;
  ...
  async getAllLeadsBySegmentationIdPaginated(id, page = 1, totalQuantity = 0) {
    const leads = await this.request('GET', `/platform/segmentations/${id}/contacts?page=${page}&page_size=125`);
    ...
    return this.getAllLeadsBySegmentationIdPaginated(id, page + 1, totalQuantity);
  }
